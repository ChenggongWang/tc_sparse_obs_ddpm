...
def plot_lost(loss_train_all, loss_valid_all, results_folder):
    
    fig, ax = plt.subplots()
    ax.plot(loss_train_all,label='train')
    ax.plot(loss_valid_all,label='validation')
    ax.set_yscale('log')
    plt.legend()
...
def train(tc_diff_model, dataloader, validation_data,val_loader, obs_ratio, results_folder, epochs):
    ...
    for epoch in range(epochs):
        for step, batch in enumerate(dataloader):
            ...
            batch_size = batch.shape[0]
            batch = batch.to(tc_diff_model.device)
            minmum_obs_ratio = 0.05 # train model with [minmum_obs_ratio,obs_ratio]*100% random sampled observation
            obs_ratio_sample = minmum_obs_ratio + (obs_ratio-minmum_obs_ratio)*np.random.rand()
            if np.random.rand()>0.95: obs_ratio_sample=0 # 10% of the time, train model with no conditions
            # Algorithm 1 line 3: sample t uniformally for every example in the batch
            t = torch.randint(0, tc_diff_model.timesteps, (batch_size,), device=tc_diff_model.device).long()
            loss = tc_diff_model.p_losses(tc_diff_model.model, batch, t, loss_type="huber", obs_ratio=obs_ratio_sample)
            if step % 200 == 0:
                ...
                plot_lost(loss_train_all, loss_valid_all, results_folder)
...
def main():
    timesteps = 300
    batch_size = 8
    results_folder = Path("./tc_guide_wvp_rand_e8_u1248")
    ...
    loss_train_all, loss_valid_all = train(
        ...
        results_folder, epochs = 8
        )
    plot_lost(loss_train_all, loss_valid_all, results_folder)

    print('Trainning is done. model are saved at: ', results_folder)
